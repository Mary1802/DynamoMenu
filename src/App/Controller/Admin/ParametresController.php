...
final class ParametresController
{
    ...
    public function handle(array $get, array $post, array $user): array
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($post['save_contact'])) {
                ...
                if ($nom === '') {
                    ...
                    $contacts->update($editingId, $nom, $adresse, $telephone, $whatsapp, $email);
                    $message = 'Contact mis à jour.';
                } else {
                    $contacts->create($nom, $adresse, $telephone, $whatsapp, $email);
                    $message = 'Contact ajouté.';
                    $editingId = 0;
                }
            }

            if (isset($post['save_horaires'])) {
                $this->settings->saveHoraires((string) ($post['horaires'] ?? ''));
        ...
        return [
            ...
            'editingId' => $editingId,
            'horaires' => $this->settings->horaires(),
        ];
    }
}
