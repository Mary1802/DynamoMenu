...
<head>
    ...
    <style>
        ...
        .add-cart.out-of-stock { opacity: 0.45; pointer-events: none; }
        .menu-card.is-out-of-stock { opacity: 0.72; }
        .menu-card-tag--stock-out { color: #f5c6cb; border-color: rgba(220,53,69,.35); }
...
<body class="client-site client-luxury">
    ...
    <script>
        ...
        function fruitOptionsFor(item) {
            if (Array.isArray(item.fruit_options) && item.fruit_options.length) {
                return item.fruit_options;
            }
            return [];
        }

        function drinkKind(item) {
            const n = (item.name || '').toLowerCase().trim();
            if (n === DRINK_SODA_NAME) return 'soda';
            if (fruitOptionsFor(item).length) return 'fruit';
            return null;
        }
        ...
        function render(filter){
            ...
            list.forEach((i, idx)=>{
                ...
                // Plats : stock null (pas de suivi). Boissons : disponible seulement si stock > 0.
                const available = i.available !== false
                    && (i.stock == null || i.stock === '' || (parseInt(i.stock, 10) || 0) > 0);
                card.className='card menu-card border-0 overflow-hidden' + (available ? '' : ' is-out-of-stock');
                ...
                const stockTag = available ? '' : '<span class="menu-card-tag menu-card-tag--stock-out">Rupture</span>';
                const cartBtn = available
                    ...
                    : `<button type="button" class="btn btn-sm btn-outline-light flex-shrink-0 add-cart out-of-stock" disabled title="Rupture de stock">Rupture</button>`;
                card.innerHTML = `
            ...
            document.querySelectorAll('.add-cart:not([disabled])').forEach(btn => {
        ...
        async function syncCartButtons() {
            ...
            for (const btn of buttons) {
                if (btn.disabled || btn.classList.contains('out-of-stock')) continue;
        ...
        async function addToCart(idx) {
            const item = currentList[idx];
            if (!item) return;
            const stockTracked = item.stock != null && item.stock !== '';
            if (item.available === false || (stockTracked && (parseInt(item.stock, 10) || 0) <= 0)) {
                showToast('Cet article est en rupture de stock.');
                return;
            }
        ...
        function openFruitModal(item) {
            ...
            const flavors = fruitOptionsFor(item);
            const list = document.getElementById('fruitFlavorList');
            list.innerHTML = flavors.map((f, i) =>
